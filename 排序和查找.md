...
private:
    int partition(vector<string> &strs,int left,int right){
        ...
        while(left<right){
            while(left<right && key+strs[right] >= strs[right]+key)
                right--;
            strs[left]=strs[right];
            while(left<right && strs[left]+key >= key+strs[left] )
                left++;
...
**[题目](https://leetcode-cn.com/problems/sort-list/)**：插给你链表的头结点 head ，请将其按 升序 排列并返回 排序后的链表 。
...
private:
    ...
    //获取中点的指针，以及 [head,mid]的节点数量
    tuple<ListNode*,int> GetMiddle(ListNode* head,int len){
        if(len==1){
            return {head,1};
        }
        else{
    ...
    ///合并两个链表
    ListNode* Merge(ListNode *left,ListNode* right,int left_len,int right_len){
        if(left_len==0){
            return right;
        }
        else if(right_len==0){
    ...
    ListNode* MergeSort(ListNode *left,int len){
        ListNode* head = left;
        if(len>1){
            ///找到中点
            auto [mid,left_size] = GetMiddle(left,len);
            auto right = mid->next;
            int right_size = len-left_size;
            mid->next = nullptr;//将左右两边断开
            ///递归划分
            if(left_size>1)
                left=MergeSort(left,left_size);
            if(right_size>1)
                right=MergeSort(right,right_size);
            ///合并链表
            head = Merge(left,right,left_size,right_size);
        }
        return head;
    }
...
public:
    ListNode* sortList(ListNode* head) {
        ...
        while(next){
            ...
            curr->next = nullptr;
            //将curr插入到已排序好的链表中
            if(curr->val <= new_head->val){//比已存在的链表的所有节点都小
                curr->next = new_head;
                new_head = curr;
            }
            else{
                ...
                if(!(new_node->next)){ //比已存在的链表的所有节点都大
                    new_node->next = curr;
...
用 subLength 表示每次需要排序的子链表的长度，初始时 subLength=1。每次将链表拆分成若干个长度为 subLength 的子链表（最后一个子链表的长度可以小于 subLength），按照每两个子链表一组进行合并，合并后即可得到若干个长度为 subLength×2 的有序子链表（最后一个子链表的长度可以小于 subLength×2）。合并两个子链表仍然使用「21. 合并两个有序链表」的做法。
...
>
> 如果每个长度为 subLength 的子链表已经有序，合并两个长度为subLength 的有序子链表，得到长度为 subLength×2 的子链表，一定也是有序的。
>
...
**[题目](https://leetcode-cn.com/problems/find-smallest-letter-greater-than-target/)**：给你一个排序后的字符列表 letters ，列表中只包含小写英文字母。另给出一个目标字母 target，请你寻找在这一有序列表里比目标字母大的最小字母。
