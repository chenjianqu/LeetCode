...
局部最优：每次合并都取最⼤的右边界，这样就可以合并更多的区间了，整体最优：合并所有重叠的区间。
...
局部最优：遇到num[i] > num[i+1]的情况，让num[i] --，然后num[i+1]给为9，可以保证这两位变成最⼤单调递增整数。
...
卖出⽇期：这个就不好算了，但也没有必要算出准确的卖出⽇期，只要当前价格⼤于（最低价格+⼿续费），就可以收获利润，⾄于准确的卖出⽇期，就是连续收获利润区间⾥的最后⼀天（并不需要计算是具体哪⼀天）。
