...
**[题目](https://leetcode-cn.com/problems/insert-interval/)**：给你一个 无重叠的 ，按照区间起始端点排序的区间列表。
...
遍历原数组中的区间，设当前遍历到区间为p，新插入的区间为n：

* 当p的右边 小于 n的左边时，表明p与n不相交，此时将p放入新数组；

* 当p的左边 大于 n的右边时，表明p与n不相交，此时先将n放入新数组，再将p放入新数组；

* 否则，p和n相交，将p和n合并，合并后的新区间记为n；

* 为了防止n重复放入或未放入新数组，设置一个标志位。
...
vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& newInterval) {
    ...
    for(vector<int> &p : intervals){
        if(p[1] < newInterval[0]){
            ...
            newInterval[0] = std::min(newInterval[0],p[0]);
            newInterval[1] = std::max(newInterval[1],p[1]);
        }
    }
    if(!is_insertd){
        arr.push_back(newInterval);
